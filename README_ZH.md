...
TrayX 已经完成桌面端托盘常驻、隐藏恢复和运行时诊断的核心闭环。当前发布目标是首个 `1.0.0` GitHub Release，范围会继续刻意收敛在桌面端稳定性、可观测性和可恢复性上。

下面的清单用于标记当前已完成的能力，以及后续会继续打磨的方向。
...
- macOS 使用运行时生成的内嵌 template 菜单栏图标，因此发布构建不再依赖额外的托盘图片文件。
- macOS 交互保持左键切换显隐、右键打开托盘菜单。
...
手动安装或使用 GitHub Release 资产时：

1. 创建或打开 `<vault>/.obsidian/plugins/trayx/`。
2. 将 `main.js`、`manifest.json` 和 `styles.css` 复制到该目录。
3. 重新加载 Obsidian，并在这个库的社区插件设置中启用 **TrayX**。
...
| 启动时隐藏 | 前台模式下启动后最小化窗口，后台模式下启动后隐藏窗口。 |
...
在打包或上传发布资产之前，应先通过构建重新生成 `main.js`，再用 `release:check` 确认发布资产完整，且 `main.js` 仍未被 Git 跟踪。
...
- `Cmd+Q` 和应用菜单退出行为正常
- 重复关闭和恢复不会创建重复托盘
- 在 macOS 上，运行时生成的 template 托盘图标状态正常
- 在 Windows 上，小尺寸托盘图标仍然清晰可辨

上述验证完成后，再继续：

- 创建并推送带注释的 release tag，例如 `1.0.0`
- 使用干净构建产物上传已经验证过的发布资产
